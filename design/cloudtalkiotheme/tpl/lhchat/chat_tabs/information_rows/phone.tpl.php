<?php if (isset($orderInformation['phone']['enabled']) && $orderInformation['phone']['enabled'] == true) : ?>
    ...
    <div class="col-6 pb-1" ng-non-bindable>
        <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhcloudtalkio','use_phone')) : ?>
            <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhcloudtalkio','use_unhidden_phone')) : ?><a href="tel:<?php echo $chat->phone?>"><?php endif; ?><span class="material-icons" >dialpad</span>
            ...
            <?php endif; ?>
            <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhcloudtalkio','use_unhidden_phone')) : ?></a><?php endif; ?>
        <?php else : ?>
            <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhcloudtalkio','use_unhidden_phone')) : ?>
                <?php echo htmlspecialchars($chat->phone)?>
            <?php else : ?>
        ...
        <br/><span class="text-primary action-image" data-phone="<?php echo htmlspecialchars(erLhcoreClassModule::getExtensionInstance('erLhcoreClassExtensionCloudtalkio')->encryptPhone($chat->phone))?>" id="chat-cloudtalk-direct-<?php echo $chat->id?>" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('cloudtalkio/admin','We will start call directly')?>"><span class="material-icons" >phone</span>
                <?php if (erLhcoreClassUser::instance()->hasAccessTo('lhcloudtalkio','use_unhidden_phone')) : ?>
                    <?php echo htmlspecialchars($chat->phone)?>
                <?php else : ?>
                    <?php echo htmlspecialchars(LiveHelperChat\Helpers\Anonymizer::maskPhone($chat->phone))?>
    ...
    <div class="col-6 pb-1" ng-non-bindable>
        ...
        <?php endif; ?>
        <span class="text-primary action-image d-block" data-phone="<?php echo htmlspecialchars(erLhcoreClassModule::getExtensionInstance('erLhcoreClassExtensionCloudtalkio')->encryptPhone($chat->phone))?>" id="chat-cloudtalk-updatenumber-btn-right-<?php echo $chat->id?>" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('cloudtalkio/admin','We will send a widget to update phone number.')?>"><span class="material-icons" title="<?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('chat/adminchat','Phone')?>">phone</span> <?php echo erTranslationClassLhTranslation::getInstance()->getTranslation('cloudtalkio/admin','Invite to update number')?></span>
